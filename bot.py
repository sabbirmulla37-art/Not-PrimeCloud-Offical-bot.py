...
async def get_sshx_session(container_name):
    ...
    for _ in range(20):
        ...
        # Strip ANSI terminal escape sequences that sshx can append to the URL
        # (for example: \x1b[0m), then extract only the clean URL.
        clean_out = re.sub(r'\x1b\[[0-9;]*[A-Za-z]|\[0m', '', out or '')
        match = re.search(r"https://sshx\.io/[^\s]+", clean_out)
        if match:
